...
public class LoginController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            AccountDTO account = accountDAO.checkLogin(id, passwordHash);
            if (account != null) {
                session.setAttribute("USER", account);
                Date date = new Date();
                LOGGER.info("User " + account.getId() + " has logged in at " + date);
            } else {
                request.setAttribute("LOGIN_FAIL", "Wrong user id or password");
                url = LOGIN_PAGE;
            }
        } catch (Exception e) {
            LOGGER.error(e);
